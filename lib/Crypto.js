...
var Crypto = function () {
  ...
  _createClass(Crypto, null, [{
    ...
    value: function createHash(data) {
      ...
      var byteArray = bc.hexStringToByte(data);
      return _crypto2.default.createHash(type).update(byteArray).digest().toString('hex');
    }
  }, {
    ...
    value: function createSHA256Hash(data) {
      // create byte array from string
      var byteArray = bc.hexStringToByte(data);
      return _crypto2.default.createHash('sha256').update(byteArray).digest().toString('hex');
    }
  }, {
    ...
    value: function createRIPEMD160Hash(data) {
      // create byte array from string
      var byteArray = bc.hexStringToByte(data);
      return _crypto2.default.createHash('ripemd160').update(byteArray).digest().toString('hex');
    }
  }, {
    key: 'randomBytes',
    ...
    key: 'randomBytesHex',
    value: function randomBytesHex() {
      var size = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 16;

      return _crypto2.default.randomBytes(size).toString('hex');
    }
  }, {
    key: 'randomBytesBuffer',
    value: function randomBytesBuffer() {
      var size = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 16;
