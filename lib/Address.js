...
var Address = function () {
  ...
  _createClass(Address, [{
    ...
    value: function details(address) {
      return _axios2.default.get('https://explorer.bitcoin.com/api/bch/addr/' + _bchaddrjs2.default.toLegacyAddress(address)).then(function (response) {
        delete response.data.addrStr;
        response.data.legacyAddress = _bchaddrjs2.default.toLegacyAddress(address);
        response.data.cashAddress = _bchaddrjs2.default.toCashAddress(address);
    ...
    value: function utxo(address) {
      return _axios2.default.get('https://explorer.bitcoin.com/api/bch/addr/' + _bchaddrjs2.default.toLegacyAddress(address) + '/utxo').then(function (response) {
        response.data.forEach(function (data) {
          delete data.address;
          data.legacyAddress = _bchaddrjs2.default.toLegacyAddress(address);
          data.cashAddress = _bchaddrjs2.default.toCashAddress(address);
        });
        return response.data;
      }).catch(function (error) {
        return JSON.stringify(error.response.data.error.message);
