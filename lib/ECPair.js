...
var ECPair = function () {
  ...
  _createClass(ECPair, null, [{
    ...
    key: 'signHex',
    value: function signHex(ecpair, hex) {
      return ecpair.sign(Buffer.from(hex, 'hex'));
    }
  }, {
    key: 'signBuffer',
    value: function signBuffer(ecpair, buffer) {
      return ecpair.sign(buffer);
    }
  }, {
    key: 'verifyHex',
    value: function verifyHex(ecpair, hex, signature) {
      return ecpair.verify(Buffer.from(hex, 'hex'), signature);
    }
  }, {
    key: 'verifyBuffer',
    value: function verifyBuffer(ecpair, buffer, signature) {
